...
gp <- ggplot(data = ddd) + 
 # geom_violin(aes(x = canton, y = prime, group = canton), linetype = "blank", fill = "#e5dbcd", alpha = 0.6) + 
  geom_line(data = ddd %>% group_by(canton) %>% summarise(prime = median(prime)) %>% ungroup(), aes(x = canton, y = prime, group = 1), size = 0.6, alpha = 0.5, colour = "darkgrey") + 
  ...
  #geom_point(aes(x = canton, y = prime, group = insurerName, color = insurerRank), size = 0.6, alpha = 0.5, shape = 18) + 
  ...
  geom_text(data  = ddd %>% filter(Kanton == "GE"), aes(x = canton, y = prime, label = insurerName), nudge_x = 0.35, hjust = 0, size = 1.9, family = "OpenSans-CondensedLight", check_overlap = T, colour = "#595959") +
  swi_theme(y_gridlines = F, base_size = 12) + theme(
  ...
  scale_x_discrete(expand = c(-1, 2), name = "") +
  scale_y_continuous(expand = c(0.005,0.01), breaks = scales::pretty_breaks(n =5))
...
save_html(
  tags$html(
    ...
    tags$body(    
      ...
      HTML(iframeresizer)  
    )), file = paste0("Primes_2017_byCantonInsurer", lang, ".html"), libdir = "js", background = "#f3f3f2"
)
