...
ddd <- c(ddd, ddd[length(ddd)], ddd[length(ddd)], ddd[length(ddd)], ddd[length(ddd)], ddd[length(ddd)], ddd[length(ddd)], ddd[length(ddd)],  ddd[length(ddd)],  ddd[length(ddd)])
...
for(lang in colnames(txt)) {
  ...
  p <- ggplot(
    ...
    geom_text(
      ...
      hjust = 0, nudge_x = 100, family = txt['base.font', lang], size = 5, alpha  = 0.9
    ) +
    ...
    theme(plot.margin = unit(c(0.3, 0, 0.7, 0.3), "lines")) +
    scale_x_continuous(
    ...
    scale_color_manual(values = swi_rpal) +
    labs(
      title = paste0(strwrap(txt['main.title', lang], width = 60, prefix = "\n", initial = ""), collapse = ""), 
      subtitle =  txt['descr', lang],
      caption = paste0(txt['source', lang], ": ", txt['source.name', lang], " | ", txt['footer', lang], " | swissinfo.ch")
      )
