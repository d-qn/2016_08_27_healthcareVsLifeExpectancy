...
#dd.point[which(dd.point$year == min(tRanges)), 'col'] <- "white" 
#dd.point [which(dd.point$year == max(tRanges)), 'col'] <- "black" 
...
for(lang in colnames(txt)) {
  ...
  footer2 <-paste0(strwrap(footer2, width = 190, prefix = "\n"), collapse = "")
  ...
  p <- ggplot(
    ...
    geom_text( 
      ...
      nudge_x = 50, nudge_xnudge_y = 1.2, alpha = 0.9,
      family = txt['title.font', lang], size = 4
    ) + 
    geom_segment(
      data = dd.seg, 
      aes(x = x, xend = xend, y = y, yend = yend, 
          group = iso2c, colour = col), size = 0.3, alpha = 0.8
    ) +
    geom_line(data = dd,
      aes(x = he, y = le, group = iso2c, colour = col), 
      size = 0.3, linetype = "dashed", alpha = 0.9
    ) + 
    geom_point(aes(colour = col, group = iso2c, frame = .frame), size = 3.8, shape = 18, alpha = 0.9) +
    geom_text(
      data = time.lab, aes(x = x, y = y, label = year, frame  = .frame),
      size = 80, hjust = 1, vjust = 0, family = txt['title.font', lang],
      colour = "#e5dbcd", alpha = 0.3)   +
    sTheme(
      base_size = 20, base_family = txt['base.font', lang], 
      title_family =  txt['title.font', lang], 
      subtitle = txt['base.font', lang]
    ) +
    theme(plot.margin = unit(c(0.3, 1.5, 0.2, 0.7), "lines")) +
    scale_x_continuous(
    ...
    labs(
      title = paste0(strwrap(txt['main.title', lang], width = 66, 
        prefix = "\n", initial = ""), collapse = ""),
  ...
  #     data = time.lab, aes(x = x, y = y, label = year, frame  = .frame), 
  #     size = 80, hjust = 1, vjust = 0, family = txt['title.font', lang], 
  #     colour = "#e5dbcd", alpha = 0.4)   +
  #   geom_text(
  #     aes(x = he, y = le, label = label, frame  = .frame, colour = iso2c), 
  #     hjust = 0, nudge_x = 30, nudge_y = 0.3, family = txt['base.font', lang], size = 5, alpha  = 0.9
  #   ) +
  #   geom_path(aes( group = iso2c, colour = iso2c, cumulative = TRUE), size = 0.3) + 
  #   geom_point(aes( group = iso2c, colour = iso2c), size = 2.5) +
  #   sTheme(base_size = 20, base_family = txt['base.font', lang], title_family =  txt['title.font', lang], 
  #          subtitle = txt['base.font', lang]) +
  #   theme(plot.margin = unit(c(0.3, 1.5, 0.2, 0.7), "lines")) +
  #   scale_x_continuous(
  #     breaks = scales::pretty_breaks(n = 10),
  ...
  #   scale_color_manual(values = swi_rpal) +
  #   labs(
  #     title = paste0(strwrap(txt['main.title', lang], width = 66, prefix = "\n", initial = ""), collapse = ""), 
  #     subtitle =  txt['descr', lang],
  #     caption = paste0(txt['source', lang], ": ", txt['source.name', lang], " | ", txt['footer.a', lang], " | swissinfo.ch", "\n", footer2)
  #     )
  # 
  animation::ani.options(interval = ani.interval)
  gg_animate(p, outfile, title_frame = F, ani.width = 640 * 1.4, 
             ani.height = 640 * 1.4)
