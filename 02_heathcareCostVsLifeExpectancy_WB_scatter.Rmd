...
iso2.label <- c("CH", "IT", "FR", "JP", "GB", "ES", "US", "CN", "DE", "TR", "MA", "PT", "BR", "RU")
...
dd$tooltip <- paste0(
  ...
  '<tr><td>', txt["y.lab2",lang], ": <strong>", round(dd$le), '</strong> ', txt["le.tp2",lang], '</td></tr>',
 # '<tr><td>', txt["gdp.tp",lang], ": ", round(dd$gdp), '</td></tr>',
  '</table>')
...
bc <- highchart(height = 580) %>%
  hc_chart(type = "bubble", spacing = c(12, 4, 4, 0)) %>%
  ...
  hc_legend(enabled = T, title = list(text = txt['legend.key', lang])) %>% 
  hc_add_theme(hc_theme_swi) %>%
   hc_colors(swi_rpal[-c(1,2,3)])
...
save_html(
  tags$html(
    ...
    tags$body(   
      ...
      div(class = "footer", txt['footer.tp', lang]),
      HTML(iframeresizer)
    )
  ), file = paste0("healthEVsLifeE_bubble_", lang, ".html"), libdir = "js", background = "#f2f2f2"
)     
